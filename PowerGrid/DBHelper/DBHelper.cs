...
namespace PowerGrid.DBHelper
{
    ...
    public sealed class DbHelper
    {
        ...
        static void Main()
        {
            ...
            //string sqlStr = @"UPDATE `User` SET `Password` = @password WHERE `Id` = @userId";
            //DbParameter[] param = new OleDbParameter[2];
            //param[0] = new OleDbParameter("@password", OleDbType.VarChar);
            //param[0].Value = "adminpassword123";
            //param[1] = new OleDbParameter("@userId", OleDbType.Integer);
            //param[1].Value = 1;
            //int i = db.ExecuteNonQuery(sqlStr, CommandType.Text, param);
            //MessageBox.Show( i + "");


            string sqlStr = @"SELECT * FROM Customer";
            DbParameter[] param = new OleDbParameter[0];
            DataTable dt = db.GetDataTable(sqlStr, CommandType.Text, param);
